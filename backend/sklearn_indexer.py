...
class SklearnIndexer:
    ...
    def search(self, query, use_pagerank,use_hits,use_vector_space, top_k=5):
        ...
        query_title = self.title_vectorizer.transform([query.lower()])
        ...
        for term, factor in self.term_boosts.items():
            ...
            if term in self.title_vectorizer.vocabulary_:
                idx = self.title_vectorizer.vocabulary_[term]
                # multiply that dimension by your boost factor
                query_title[0, idx] *= factor
                # print(query_title[0, idx])

        query_description = self.body_vectorizer.transform([query.lower()])
        ...
        if use_pagerank:
            ...
            for doc_id, _ in top_docs:
                scores[doc_id] = 0.7 * scores[doc_id] + 0.3 * self.pr_scores.get(doc_id, 0)
            ...
            # print(local_auths," = ",_,)
            for doc_id, _ in top_docs:
                scores[doc_id] = 0.7 * scores[doc_id] + 0.2 * self.authorities.get(doc_id, 0) + 0.1 * self.hubs.get(doc_id, 0)
        else:
            print("Vector Space")
